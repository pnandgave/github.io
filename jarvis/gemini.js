...
(function (root) {
  ...
  async function generate(key, model, system, text, schema) {
    const body = {
      systemInstruction: {parts: [{text: system}]},
      contents: [{role: 'user', parts: [{text}]}],
      generationConfig: {temperature: 0.1, ...(schema ? {responseMimeType: 'application/json', responseSchema: schema} : {maxOutputTokens: 300})},
    };
  ...
  async function parseCommand(key, model, text, rules) {
    ...
    const schema = {type: 'OBJECT', properties: {
      intent: {type: 'STRING', enum: ['add_task', 'brief', 'question']},
    ...
    }, required: ['intent', 'title', 'workstream', 'when', 'time', 'duration_min', 'priority']};
...
For brief or question: title '', workstream ${rules.inbox}, when '', time '', duration_min 0, priority P2.`;
  ...
  const answer = (key, model, text, rules) => generate(key, model,
    `You are JARVIS, ${rules.name}'s calm, concise personal assistant. Answer in at most 3 short sentences. ` +
    'If the question needs exact technical facts such as standards, numbers or citations, say that it should be checked against the source.', text);

  root.JGemini = {pickModel, parseCommand, answer};
})(this);
